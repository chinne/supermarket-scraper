...
class AldiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        article = response.xpath('//*[@class="mod-article-tile__title"]/text()').extract()
        price = response.xpath('//*[@class="price__main"]/text()').extract()
        description_short = response.xpath('//*[@class="mod-article-tile__info"]/p/text()').extract()
        # print(articles)
        ...
        for i, _ in enumerate(price_list):
            # print(articl)
            # print(price)
            product = article.xpath('//*[@class="mod-article-tile__title"]/text()').extract(),
            price = article.xpath('//*[@class="price__main"]/text()').extract()
            print(product_list[i].strip())
            yield {
                'Produkt': product_list[i].strip(), 'Beschreibung': description_short[i].strip(), 'Preis': price_list[i].strip()
            }
